...
class BertModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            for i in range(batch_size):
                cur_sequence_output = tf.reshape(tf.slice(self.sequence_output,[i,0,0],[1,seq_length,config.hidden_size]),shape=[seq_length,config.hidden_size])
    ...
    def get_entity_embedding(self):
        return self.entity1_embedding,self.entity2_embedding
...
def embedding_postprocessor(input_tensor,
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            position1_embeddings = tf.nn.embedding_lookup(full_position_embeddings,flat_position1_ids)
            position2_embeddings = tf.nn.embedding_lookup(full_position_embeddings,flat_position2_ids)
            # num_dims = len(output.shape.as_list())
            position_embeddings = position1_embeddings+position2_embeddings
            position_embeddings = tf.reshape(position_embeddings,shape=[batch_size,seq_length,embedding_size])
