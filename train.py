...
class InputExample(object):
    ...
    def __init__(self, guid, text, head_entity, tail_entity, label=None):
...
class NREProcessor(DataProcessor):
    def __init__(self):
        super(NREProcessor, self).__init__()
        self.labels = []
        with open(FLAGS.data_dir + 'relation2id.txt', 'r') as f:
            lines = f.readlines()
        for line in lines:
    ...
    def _create_examples(self, lines, set_type):
        """create example for the training and dev set"""
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text = tokenization.convert_to_unicode(line[4])
            label = tokenization.convert_to_unicode(line[3])
            head_entity = tokenization.convert_to_unicode(line[1])
            tail_entity = tokenization.convert_to_unicode(line[2])
            examples.append(
                InputExample(guid=guid, text=text, head_entity=head_entity, tail_entity=tail_entity, label=label)
            )
        return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    head = example.head_entity
    tail = example.tail_entity
    ...
    for i in range(len(final_tokens)):
        position1_ids.append(min((i - head_ids) + FLAGS.max_position_length, FLAGS.max_position_length * 2 - 1))
        position2_ids.append(min((i - tail_ids) + FLAGS.max_position_length, FLAGS.max_position_length * 2 - 1))

    while len(position1_ids) < max_seq_length:
        position1_ids.append(0)
        position2_ids.append(0)
...
def create_model(bert_config, is_training, input_ids, head_ids, tail_ids, position1_ids, position2_ids, input_mask,
                 segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        pos = tf.reduce_sum(abs(pos_tail_embedding + pos_head_embedding - sentence_embedding), axis=1, keep_dims=True)
        neg = tf.reduce_sum(max(abs(neg_tail_embedding + pos_head_embedding - sentence_embedding),
                                abs(pos_tail_embedding + neg_head_embedding - sentence_embedding)), axis=1,
                            keep_dims=True)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, pre_loss, logits, probabilities) = create_model(
            bert_config, is_training, num_labels, input_ids, head_ids, tail_ids, position1_ids, position2_ids,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits, is_real_example):
                ...
                recall = tf.metrics.recall(predictions=predictions, labels=label_ids, weights=is_real_example)
                predictions = tf.metrics.precision(labels=label_ids, predictions=predictions, weights=is_real_example)
            ...
            eval_metrics = (metric_fn,
                            [pre_loss, label_ids, logits, is_real_example])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
    ...
    for feature in features:
        ...
        all_position1_ids.append(feature.position1_ids)
        all_position2_ids.append(feature.position2_ids)
